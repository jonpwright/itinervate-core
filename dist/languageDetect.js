...
exports.speechLocaleFor = speechLocaleFor;
/**
...
function detectLanguage(text) {
    ...
    for (const [re, lang] of SCRIPTS)
        if (re.test(t))
            return lang === 'ru' && UK_HINT.test(t) ? 'uk' : lang;
    ...
    for (const [lang, list] of Object.entries(STOP)) {
        ...
        if (lang === 'fr' && /[éèêàçù]/.test(t))
            score += 0.5;
        if (lang === 'es' && /[ñ¿¡]/.test(t))
            score += 1;
        if (lang === 'de' && /[äöüß]/.test(t))
            score += 1;
        if (lang === 'pt' && /[ãõç]/.test(t))
            score += 1;
        if (lang === 'tr' && /[şğı]/.test(t))
            score += 1;
        if (lang === 'pl' && /[łżźćńś]/.test(t))
            score += 1;
        if (lang === 'cs' && /[řěůň]/.test(t))
            score += 1;
        if (lang === 'vi' && /[ơưđạảấ]/.test(t))
            score += 1;
        if (lang === 'ro' && /[șță]/.test(t))
            score += 1;
        if (score > bestScore) {
            second = bestScore;
            bestScore = score;
...
function speechLocaleFor(lang, preferRegion) {
    const base = lang.toLowerCase().split('-')[0];
    if (preferRegion && preferRegion.toLowerCase().startsWith(base + '-'))
        return preferRegion;
    const DEFAULTS = { en: 'en-US', ja: 'ja-JP', zh: 'zh-CN', ko: 'ko-KR', fr: 'fr-FR', es: 'es-ES', de: 'de-DE', it: 'it-IT', pt: 'pt-PT', nl: 'nl-NL', sv: 'sv-SE', da: 'da-DK', nb: 'nb-NO', fi: 'fi-FI', pl: 'pl-PL', cs: 'cs-CZ', tr: 'tr-TR', id: 'id-ID', vi: 'vi-VN', ro: 'ro-RO', ru: 'ru-RU', uk: 'uk-UA', ar: 'ar-SA', he: 'he-IL', hi: 'hi-IN', th: 'th-TH', el: 'el-GR' };
