...
export function activeFilterCount(f: MeetingFilter): number {
  let n = 0;
  if (f.query.trim()) n++;
  if (f.range !== 'all') n++;
  if (f.status !== 'all') n++;
 ...
 */
export function toDayString(date?: unknown): string {
  if (!date) return '';
 ...
 */
export function parseLocalDateTime(date?: unknown, time?: string): Date | null {
  if (!date) return null;
...
export function meetingEnd(m: any): Date | null {
...
export function isPastMeeting(m: any, now: Date): boolean {
 ...
 */
export function hasDebrief(m: any): boolean {
  const d = m?.debrief;
  if (d) {
    if (
      d.completedAt ||
 ...
 */
export function attendeeNames(m: any): string[] {
...
export function meetingHaystack(m: any): string {
  return [
    m?.title,
    m?.company,
    m?.location,
...
export function tripHaystack(t: any): string {
  return [
    t?.name,
    t?.destinationCity,
...
function daysFrom(d: Date, delta: number): Date {
...
export function rangeBounds(
  f: Pick<MeetingFilter, 'range' | 'from' | 'to'>,
  now: Date,
  trips?: any[],
  ...
  switch (f.range) {
    case 'lastTrip': {
      ...
      if (!start || !end) return { from: new Date(0), to: new Date(0) };
...
function withinBounds(
  ...
  if (bounds.from && when.getTime() < bounds.from.getTime()) return false;
  if (bounds.to && when.getTime() > bounds.to.getTime()) return false;
  return true;
}
...
function matchesStatus(m: any, status: StatusFilter, now: Date): boolean {
...
function compareMeetings(a: any, b: any, sort: SortOrder): number {
  if (sort === 'title') {
 ...
 */
export function filterMeetings<T extends Record<string, any>>(
  meetings: T[],
  f: MeetingFilter,
  now: Date,
  ...
  return meetings
    .filter((m) => {
      if (!includeArchived && isArchived(m)) return false;
      if (!matchesStatus(m, f.status, now)) return false;
      if (!inRangeOrOnTrip(m, meetingStart(m), bounds, lastTrip)) return false;
      if (query && !matchesQuery(meetingHaystack(m), query)) return false;
      return true;
    })
    .slice()
 ...
 */
export function recentMeetings<T extends Record<string, any>>(
  meetings: T[],
  now: Date,
  limit = 6,
): T[] {
  ...
  const undated: T[] = [];
  for (const m of live) {
    if (meetingStart(m) == null) undated.push(m);
    else if (isPastMeeting(m, now)) past.push(m);
...
export function meetingStatusCounts(
  meetings: any[],
  now: Date,
...
export const DEFAULT_TRIP_FILTER: TripFilter = {
  query: '',
  status: 'all',
  sort: 'newest',
};
...
export function tripStatus(
  t: any,
  now: Date,
): 'upcoming' | 'inProgress' | 'past' | 'unknown' {
  const start = parseLocalDateTime(t?.startDate);
  const end = parseLocalDateTime(t?.endDate);
  if (!start || !end) return 'unknown';
  const today = startOfDay(now).getTime();
  if (endOfDay(end).getTime() < today) return 'past';
  if (startOfDay(start).getTime() > today) return 'upcoming';
  return 'inProgress';
}

function compareTrips(a: any, b: any, sort: SortOrder): number {
  if (sort === 'title') {
...
export function filterTrips<T extends Record<string, any>>(
  trips: T[],
  f: TripFilter,
  now: Date,
  includeArchived = false,
  ...
  return trips
    .filter((t) => {
      if (!includeArchived && isArchived(t)) return false;
      if (f.status !== 'all' && tripStatus(t, now) !== f.status) return false;
      if (query && !matchesQuery(tripHaystack(t), query)) return false;
      return true;
    })
    .slice()
 ...
 */
export function lastPastTrip(trips: any[], now: Date): any | null {
  let best: any = null;
  let bestEnd = -Infinity;
  for (const t of trips) {
    if (isArchived(t) || tripStatus(t, now) !== 'past') continue;
...
function inRangeOrOnTrip(
  ...
  if (lastTrip) {
    if (record?.tripId === lastTrip.id) return true;
    if (record?.tripId) return false;
  }
...
export function activeExpenseFilterCount(f: ExpenseFilter): number {
  let n = 0;
  if (f.query.trim()) n++;
  if (f.range !== 'all') n++;
  return n;
...
export function expenseDate(e: any): Date | null {
  const day = toDayString(e?.date);
  return day ? parseLocalDateTime(day) : null;
}

export function expenseHaystack(e: any): string {
...
function compareExpenses(a: any, b: any, sort: SortOrder): number {
  if (sort === 'title') {
...
export function filterExpenses<T extends Record<string, any>>(
  ...
  return expenses
    .filter((e) => {
      if (!inRangeOrOnTrip(e, expenseDate(e), bounds, lastTrip)) return false;
      if (query && !matchesQuery(expenseHaystack(e), query)) return false;
      return true;
    })
    .slice()
 ...
 */
export function reportDate(r: any): Date | null {
  const day = toDayString(r?.startDate) || toDayString(r?.generatedAt);
  return day ? parseLocalDateTime(day) : null;
}

export function reportHaystack(r: any): string {
...
function compareReports(a: any, b: any, sort: SortOrder): number {
  if (sort === 'title') {
...
export function filterReports<T extends Record<string, any>>(
  reports: T[],
  f: ExpenseFilter,
  now: Date,
  trips?: any[],
  ...
  return reports
    .filter((r) => {
      if (!inRangeOrOnTrip(r, reportDate(r), bounds, lastTrip)) return false;
      if (query && !matchesQuery(reportHaystack(r), query)) return false;
      return true;
    })
    .slice()
 ...
 * before grouping.
 */
export function groupByTrip<T extends Record<string, any>>(
  records: T[],
  trips: any[],
  when: (r: T) => Date | null,
): TripGroup<T>[] {
  const byId = new Map<string, any>();
  for (const t of trips) if (t?.id) byId.set(t.id, t);

  const groups = new Map<string, TripGroup<T>>();
  const place = (key: string, trip: any | null, r: T) => {
  ...
  for (const r of records) {
    ...
    if (host) place(host.id, host, r);
    else place('none', null, r);
  }
...
export function tripStatusCounts(
  trips: any[],
  now: Date,
