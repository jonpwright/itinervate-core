...
export interface SpeechTextOptions {
  ...
  words?: { rows: string; andMore: string };
}

const DEFAULT_WORDS = { rows: 'rows', andMore: 'and more in the message' };

export function speechText(markdown: string, opts: SpeechTextOptions = {}): string {
  ...
  if (s.length > max) {
    const cut = s.slice(0, max); const end = Math.max(cut.lastIndexOf('. '), cut.lastIndexOf('! '), cut.lastIndexOf('? '));
