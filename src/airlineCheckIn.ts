/**
 * Airline online check-in pages, keyed by IATA carrier code.
 *
 * Duffel issues the order (PNR, e-tickets); the boarding pass comes from the
 * airline at check-in. These are the carriers' public check-in entry points.
 * Where a carrier accepts the booking reference and surname in the URL we
 * pre-fill them; otherwise the page opens ready for the traveller to type.
 * Anything not listed falls back to a web search for "<airline> online check-in".
 */
type Builder = (ref?: string, surname?: string) => string;
const q = (v?: string) => encodeURIComponent((v || '').trim());

const CHECK_IN: Record<string, Builder> = {
...
export function airlineCheckInUrl(iata?: string | null, airlineName?: string | null, bookingReference?: string | null, surname?: string | null): string {
  const code = (iata || '').trim().toUpperCase();
  const b = code ? CHECK_IN[code] : undefined;
  if (b) return b(bookingReference || undefined, surname || undefined);
...
export function hasAirlineCheckIn(iata?: string | null): boolean {
